          ...
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <a
                href={profile.resume}
                download
                className="inline-flex items-center gap-3 rounded-full border border-white/15 px-6 py-3 text-sm font-bold uppercase tracking-[0.18em] text-white/80 transition hover:border-ice-blue hover:text-ice-blue"
              >
          ...
          <TiltPanel className="rounded-[2rem] p-5">
            <div className="relative overflow-hidden rounded-[1.5rem]">
              <img
                ...
                alt="Developer desk"
                className="h-[520px] w-full object-cover opacity-74 mix-blend-luminosity"
              />
              ...
              <div className="absolute bottom-6 left-6 right-6 rounded-3xl border border-white/10 bg-black/40 p-6 backdrop-blur-xl">
                ...
                <p className="mt-2 text-white/62">{profile.intro}</p>
              </div>
            </div>
          </TiltPanel>
        </div>
          ...
          <div className="grid gap-5 md:grid-cols-4">
            {stats.map((stat, index) => (
              ...
              >
                ...
                <p className="mt-2 font-mono text-xs uppercase tracking-[0.24em] text-white/42">{stat.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {chapters.map((chapter, index) => {
              ...
              return (
                ...
                >
                  <div className="mb-10 flex h-14 w-14 items-center justify-center rounded-2xl bg-ice-blue text-graphite">
                    <Icon />
                  </div>
                  <h3 className="font-display text-3xl font-bold">{chapter.title}</h3>
                  <p className="mt-4 leading-7 text-white/62">{chapter.copy}</p>
                </motion.div>
              );
            })}
          </div>
      ...
      <section className="px-4 pb-28 md:px-6">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
            ...
            <div className="mt-8 space-y-5">
              {experiences.map((item) => (
                <div key={`${item.company}-${item.period}`} className="border-l border-ice-blue/25 pl-5">
                  ...
                  <p className="text-white/58">{item.company}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 md:p-10">
            ...
            <div className="mt-8 space-y-5">
              {education.map((item) => (
                <div key={item.degree} className="rounded-2xl bg-white/[0.04] p-5">
                  <p className="font-mono text-xs uppercase tracking-[0.22em] text-white/42">{item.period}</p>
                  <h3 className="mt-2 font-display text-2xl font-bold">{item.degree}</h3>
                  <p className="text-white/58">{item.field} - {item.institution}</p>
                </div>
              ))}
            </div>
          </div>
