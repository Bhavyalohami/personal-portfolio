    ...
    <PageShell>
      <section className="px-4 pb-24 pt-32 md:px-6">
        <div className="mx-auto max-w-7xl">
            ...
            {education.map((item, index) => (
              <TiltPanel key={item.degree} className="rounded-[2rem] p-7">
                <motion.div variants={reveal} initial="hidden" animate="visible" custom={index * 0.08}>
                  <div className="mb-10 flex h-16 w-16 items-center justify-center rounded-2xl bg-ice-blue text-2xl text-graphite">
                    <FaGraduationCap />
                  </div>
                  ...
                  <p className="mt-2 text-xl text-white/72">{item.field}</p>
                  <p className="mt-2 text-white/45">{item.institution} - {item.location}</p>
                  <div className="mt-8 flex flex-wrap gap-2">
                    {item.notes.map((note) => (
                      <span key={note} className="rounded-full border border-white/10 px-3 py-2 text-sm text-white/58">
                        {note}
                      </span>
                    ))}
                  </div>
          ...
          <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 md:p-12">
